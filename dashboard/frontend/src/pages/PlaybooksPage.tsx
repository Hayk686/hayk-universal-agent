...
export function PlaybooksPage() {
  ...
  async function load() {
    try {
      const r = await getJson<FileRow[]>("/api/playbooks");
      setRows(r);
      setErr(null);
    } catch (e) {
  ...
  async function create() {
    ...
    try {
      const res = await postJson("/api/playbooks", { name });
  ...
  async function confirmDelete() {
    ...
    try {
      const res = await del(
        `/api/playbooks/${encodeURIComponent(pendingDelete)}`,
      );
      if (!res.ok) throw new Error(await res.text());
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
          Markdown files in <code className="text-xs">playbooks/</code>. Saving creates a
          timestamped backup.
        </p>
      </div>
